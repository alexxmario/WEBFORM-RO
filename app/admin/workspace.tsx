...
export function AdminWorkspace() {
  ...
  return <div className="admin-app">
    ...
    <main className="admin-main"><header className="admin-top"><span>WEBFORM / OPERAȚIUNI</span><Link href="/account">Contul meu ↗</Link></header>
      ...
      <div className="admin-stats">{["Clienți","Proiecte deschise","Plăți în așteptare","Cereri și interes","Mesaje noi"].map((label,index)=><article key={label}><span>{label}</span><strong>{counts[index]??"—"}</strong><small>{index===2?"Necesită confirmare NETOPIA":"În baza de date"}</small></article>)}</div>
