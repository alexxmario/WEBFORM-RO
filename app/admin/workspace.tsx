...
import { ArrowUpRight, Users, Layers, CreditCard, Inbox, MessageCircle, RefreshCw, X, Mail } from "lucide-react";
import "./workspace.css";

type Row = Record<string, unknown> & {id:string};
type View = "submissions" | "projects" | "clients" | "orders" | "leads" | "contacts";
const views = [{id:"submissions",label:"Toate cererile",icon:Inbox},{id:"projects",label:"Proiecte",icon:Layers},{id:"clients",label:"Clienți",icon:Users},{id:"orders",label:"Comenzi",icon:CreditCard},{id:"contacts",label:"Mesaje contact",icon:Mail},{id:"leads",label:"Cereri și interes",icon:Inbox}] as const;
...
export function AdminWorkspace() {
  ...
  useEffect(()=>{
    ...
    fetch(`/api/admin?view=${view}&page=${page}`,{signal:controller.signal,cache:"no-store"}).then(async response=>{
      ...
      if(!controller.signal.aborted) { setRows(data.rows);setTotal(data.total);setCounts(data.counts);setSubmissionCounts(data.submissionCounts || []); }
  ...
  useEffect(() => {
    const timer = window.setInterval(() => { if (document.visibilityState === "visible") setRefresh(value => value + 1); }, 30000);
  ...
  return <div className="admin-app">
    <aside className="admin-sidebar"><Link href="/" className="admin-brand"><span>w.</span> webform</Link><p className="admin-overline">SPAȚIU DE ADMINISTRARE</p><nav aria-label="Administrare">{views.map(item=><button key={item.id} aria-current={view===item.id?"page":undefined} onClick={()=>{setView(item.id);setPage(1);setSelected(null);}}><item.icon size={18}/>{item.label}</button>)}<Link href="/admin/leads"><Inbox size={18}/>Lead-uri campanie</Link><Link href="/chat"><MessageCircle size={18}/>Conversații<ArrowUpRight size={14}/></Link></nav><Link href="/" className="admin-back">Vezi site-ul <ArrowUpRight size={16}/></Link></aside>
    <main className="admin-main"><header className="admin-top"><span>WEBFORM / OPERAȚIUNI</span><Link href="/account">Contul meu ↗</Link></header>
      <div className="admin-title"><div><p className="admin-overline">TOTUL, ÎNTR-UN SINGUR LOC</p><h1>{views.find(item=>item.id===view)?.label}</h1><p>Toate formularele site-ului. Lista se actualizează automat la 30 de secunde.</p></div><button onClick={()=>setRefresh(value=>value+1)} disabled={loading} className="admin-button"><RefreshCw size={16}/>Actualizează</button></div>
      {view === "submissions" ? <div className="admin-stats">{["Homepage și instalatori", "Contact și oferte", "Briefuri de proiect"].map((label, index) => <article key={label}><span>{label}</span><strong>{submissionCounts[index] ?? "—"}</strong><small>Cereri salvate</small></article>)}</div> : <div className="admin-stats">{["Clienți","Proiecte deschise","Plăți în așteptare","Cereri și interes","Mesaje noi"].map((label,index)=><article key={label}><span>{label}</span><strong>{counts[index]??"—"}</strong><small>{index===2?"În așteptarea confirmării plății":"În baza de date"}</small></article>)}</div>}
      ...
      {error?<div className="admin-empty" role="alert"><h3>Datele nu au putut fi încărcate</h3><p>{error}</p><button className="admin-button" onClick={()=>setRefresh(value=>value+1)}>Încearcă din nou</button></div>:loading?<div className="admin-empty" role="status">Se încarcă datele…</div>:rows.length===0?<div className="admin-empty"><Inbox size={30}/><h3>Nicio înregistrare încă</h3><p>Înregistrările vor apărea aici după ce sunt create pe site.</p></div>:<div className="admin-table-wrap"><table><thead><tr><th>{view==="orders"?"Comandă":"Nume / afacere"}</th><th>{view==="orders"?"Valoare":"Detalii"}</th><th>{view === "submissions" ? "Formular" : "Stare / plan"}</th><th>Creat la</th><th><span className="sr-only">Acțiuni</span></th></tr></thead><tbody>{rows.map(row=><tr key={`${row.submission_kind || view}-${row.id}`}><td><strong>{str(row.business_name||row.name||row.email||row.id.slice(0,8))}</strong><small>{str(row.phone||row.email||row.user_id||row.id)}</small></td><td>{view==="orders"?`${str(row.amount)} ${str(row.currency)}`:str(row.submission_details||row.subject||row.one_liner||row.subscription_plan||row.business_type)}</td><td><span className="admin-badge">{view==="submissions"?str(row.submission_source):view==="contacts"?"Nou":statuses[str(row.workflow_status||row.status||row.subscription_status)]||str(row.tier||row.role)}</span></td><td>{date(row.created_at)}</td><td><button className="admin-open" onClick={()=>open(row)} aria-label={`Deschide ${str(row.business_name||row.name||row.id)}`}>Detalii ↗</button></td></tr>)}</tbody></table></div>}
      ...
      {view === "submissions" && selected.submission_kind === "campaign" && <Link className="admin-button" href="/admin/leads">Gestionează cererile de pe homepage și instalatori ↗</Link>}
