...
type View = "projects" | "clients" | "orders" | "leads" | "contacts";
const views = [{id:"projects",label:"Proiecte",icon:Layers},{id:"clients",label:"Clienți",icon:Users},{id:"orders",label:"Comenzi",icon:CreditCard},{id:"contacts",label:"Mesaje contact",icon:Mail},{id:"leads",label:"Cereri și interes",icon:Inbox}] as const;
...
export function AdminWorkspace() {
  ...
  return <div className="admin-app">
    ...
    <main className="admin-main"><header className="admin-top"><span>WEBFORM / OPERAȚIUNI</span><Link href="/account">Contul meu ↗</Link></header>
      ...
      <div className="admin-stats">{["Clienți","Proiecte deschise","Plăți în așteptare","Cereri și interes","Mesaje noi"].map((label,index)=><article key={label}><span>{label}</span><strong>{counts[index]??"—"}</strong><small>{index===2?"În așteptarea confirmării plății":"În baza de date"}</small></article>)}</div>
